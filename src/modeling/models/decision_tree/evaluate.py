...
def evaluate(database_id,
             ...
             save_results=False,
             model_group='N/A'):
    model = train.train(x_train, y_train, search=search, max_depth=max_depth, max_leaf_nodes=max_leaf_nodes)
    y_pred, y_pred_scores = test.predict(x_test, model)
    ...
    # report
    report = EvaluationReport(y_test, y_pred, y_pred_scores_target, extra_features_test, model, feature_names, database_id, algorithm=ALGORITHM_NAME)
