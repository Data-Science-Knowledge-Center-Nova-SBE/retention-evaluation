...
def run_train(df, docs, drop_first_ohe=False):
    ...
    print("df shape prior to dropping cols", df.shape)
    df = transformations.t01_drop_columns.run(df, docs, drop_columns)
    print("df shape prior to dropping null", df.shape)
    df = transformations.t05_drop_null.run(df, docs)
    print("df shape after dropping null", df.shape)
...
def run_for_all_rows(df, docs):
    ...
    df = transformations.t0802_target_creation_6_grade.run(df, docs)
...
def run(dataframe, dataset_beginning=1998, dataset_end=2019, prediction_window=1, train_history=5, test_history=1,
    ...
    if save:
        ...
        dataset_manager.save("pipeline 6_1", df, docs_train, group=group, save_duplicate=False)

        for i in df_full:
            counter += 1

            dataset_manager.save("pipeline 6_1", i['df_train_folds'], i['docs_train'], train="1", fold=str(counter),group=group, save_duplicate=False)
            dataset_manager.save("pipeline 6_1", i['df_test_folds'], i['docs_test'], train="0", fold=str(counter),group=group, save_duplicate=False)
...
def run_production(df):
    ...
    df = transformations.t03_transform_lingua_materna.run(df, docs)
    df = transformations.t04_one_hot_encode.run(df, docs, "anoEscMae", False)
    # df = transformations.t09_calc_average_nuclear.run(df, [docs])
    df = transformations.t04_one_hot_encode.run(df, docs, "nacionalidade")
    try:
        df = transformations.t04_one_hot_encode.run(df, docs, "distrito_agrup")
