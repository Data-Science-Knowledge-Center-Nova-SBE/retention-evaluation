...
def run_for_all_rows(df, docs):
    ...
    df = transformations.t0801_target_creation_8_grade.run(df, docs)
...
def run(dataframe,
    ...
    if save:
        ...
        dataset_manager.save("pipeline 8_1", df, docs_train, group=group, save_duplicate=False)

        for i in df_full:
            counter += 1

            dataset_manager.save("pipeline 8_1", i['df_train_folds'], i['docs_train'], train="1", fold=str(counter),
                                 group=group, save_duplicate=False)
            dataset_manager.save("pipeline 8_1", i['df_test_folds'], i['docs_test'], train="0", fold=str(counter),
                                 group=group, save_duplicate=False)
